...
from func.funasr.funasr_core import FunASRCore
...
def main():
    ...
    joycaption_config = config.get('joycaption', {})
    joycaption_enabled = joycaption_config.get('enabled', False)
    funasr_config = config.get('funasr', {})
    funasr_enabled = funasr_config.get('enabled', False)

    # 如果启用JoyCaption，启动独立后台线程
    if joycaption_enabled:
        joycaption_core = JoyCaptionCore()
        joycaption_core.start_background_thread()
        log.info("已启用JoyCaption独立后台线程")
    ...
    if funasr_enabled:
        funasr_core = FunASRCore()  # 不传 callback，自动使用 HTTP 调用 /msg
        funasr_core.start()
        log.info("已启用 FunASR 语音识别后台线程")
