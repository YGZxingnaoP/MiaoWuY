...
class NsfwCore:
    ...
    def nsfw_fun(self, imgb64, prompt, username, retryCount, tip, nsfw_limit):
        ...
        except RequestException as e:  # 捕获连接超时、拒绝连接等网络异常
            self.log.warning(f"《{prompt}》【nsfw】鉴黄{tip}本地服务连接失败，已跳过鉴黄。错误：{e}")
            return 1, 0  # 跳过鉴黄，默认返回“通过”及nsfw分数0
        except Exception as e:
            self.log.exception(f"《{prompt}》【nsfw】鉴黄{tip}发生了其他异常：")
            return -1, -1
        finally:
